...
from torch import optim, nn          # 优化器与模块
...
from dataset.lm_dataset import SFTDataset  # LoRA 也用对话数据微调
from model.model_lora import save_lora, apply_lora  # LoRA 挂载与（仅）保存 LoRA 权重
...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):  # 与全参微调流程相同，但只更新/裁剪/保存 LoRA 参数
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:           # 混合精度前向（注意：前向走的是“原始权重 + LoRA 旁路”）
            res = model(input_ids, labels=labels)
            loss = res.loss + res.aux_loss
            loss = loss / args.accumulation_steps

        scaler.scale(loss).backward()  # 反向（只有 LoRA 参数 requires_grad=True，故只有它们产生梯度）

        if step % args.accumulation_steps == 0:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(lora_params, args.grad_clip)  # 只裁剪 LoRA 参数的梯度
            scaler.step(optimizer)
            scaler.update()
            optimizer.zero_grad(set_to_none=True)

        if step % args.log_interval == 0 or step == iters:  # 打日志
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():  # 保存
            ...
            lora_save_path = f'{args.save_dir}/{args.lora_name}_{lm_config.hidden_size}{moe_suffix}.pth'  # LoRA 权重路径
            # LoRA只保存LoRA权重
            save_lora(model, lora_save_path)  # 仅导出 LoRA 旁路参数（文件很小）
            lm_checkpoint(lm_config, weight=args.lora_name, model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints')  # 续训断点
            model.train()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind LoRA Fine-tuning")  # LoRA 微调
    ...
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)  # 先加载基模（默认 full_sft）
    apply_lora(model)                # 给方阵 Linear 挂上 LoRA 旁路并改写 forward

    # 统计参数
    total_params = sum(p.numel() for p in model.parameters())  # 总参数
    lora_params_count = sum(p.numel() for name, p in model.named_parameters() if 'lora' in name)  # LoRA 参数
    Logger(f"LLM 总参数量: {total_params / 1e6:.3f} M")
    Logger(f"LoRA 参数量: {lora_params_count / 1e6:.3f} M")
    Logger(f"LoRA 参数占比: {lora_params_count / total_params * 100:.2f}%")  # 通常只有百分之几
    ...
    # ========== 6. 定义数据和优化器 ==========
    train_ds = SFTDataset(args.data_path, tokenizer, max_length=args.max_seq_len)  # 对话数据
    train_sampler = DistributedSampler(train_ds) if dist.is_initialized() else None
    scaler = torch.cuda.amp.GradScaler(enabled=(args.dtype == 'float16'))
    optimizer = optim.AdamW(lora_params, lr=args.learning_rate)  # 优化器只优化 LoRA 参数

    # ========== 7. 从ckp恢复状态 ==========
    start_epoch, start_step = 0, 0
    if ckp_data:
        model.load_state_dict(ckp_data['model'], strict=False)  # 宽松加载（断点里可能只有 LoRA 相关）
    ...
    # ========== 8. 编译和分布式包装 ==========
    if args.use_compile == 1:        # LoRA 通过 monkey-patch 改写了 forward，与 compile 冲突
        args.use_compile = 0
