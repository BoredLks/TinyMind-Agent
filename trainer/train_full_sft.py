...
from dataset.lm_dataset import SFTDataset  # 监督微调数据集（仅对助手回答计损失）
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):  # 训练一个 epoch（与预训练流程相同，区别在于数据集只在助手回答处算损失）
    start_time = time.time()
    last_step = start_step
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        input_ids = input_ids.to(args.device)
        labels = labels.to(args.device)  # 标签里非助手回答的位置已被置 -100，不计损失
        last_step = step
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():  # 保存
            ...
            torch.save({k: v.half().cpu() for k, v in state_dict.items()}, ckp)  # 保存纯权重（供推理）
            lm_checkpoint(lm_config, weight=args.save_weight, model=model, optimizer=optimizer,
                         epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints', scaler=scaler)  # 保存续训断点
            model.train()
            del state_dict

        del input_ids, labels, res, loss

    if last_step > start_step and last_step % args.accumulation_steps != 0:  # 处理尾部不满累积步
        scaler.unscale_(optimizer)
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
        scaler.step(optimizer)
        scaler.update()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind Full SFT")  # 全参数监督微调
    ...
    # ========== 1. 初始化环境和随机种子 ==========
    local_rank = init_distributed_mode()  # 初始化 DDP
    ...
    # ========== 5. 定义模型、数据、优化器 ==========
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)  # 默认从 pretrain 权重热启动
    train_ds = SFTDataset(args.data_path, tokenizer, max_length=args.max_seq_len)  # 监督微调数据集
