...
from transformers import AutoModel  # （奖励模型用）
from model.model_minimind import MiniMindConfig, MiniMindForCausalLM  # 模型
from dataset.lm_dataset import RLAIFDataset  # 在线 RL 数据集
...
from trainer.rollout_engine import create_rollout_engine  # 采样引擎

warnings.filterwarnings('ignore')


def rep_penalty(text, n=3, cap=0.5):  # 重复惩罚（同 PPO）：n-gram 重复越多惩罚越大
    toks = re.findall(r"\w+|[^\w\s]", text.lower())
    grams = [tuple(toks[i:i + n]) for i in range(len(toks) - n + 1)]
    return min(cap, (len(grams) - len(set(grams))) * cap * 2 / len(grams)) if grams else 0.0


def calculate_rewards(prompts, responses, reward_model):  # 计算每条回复奖励（每个 prompt 有 num_generations 条）
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):  # 遍历每个 prompt
            for j in range(args.num_generations):  # 遍历该 prompt 的每条生成
                response_idx = i * args.num_generations + j  # 在扁平数组中的下标
                response = responses[response_idx]
                prompt = prompts[i]

                pattern = r"<\|im_start\|>(system|user|assistant)\s+(.*?)<\|im_end\|>"  # 还原对话
                matches = re.findall(pattern, prompt, re.DOTALL)
                messages = [{"role": role, "content": content.strip()} for role, content in matches]
                answer = response
                rewards[response_idx] += 0.5 if 20 <= len(response.strip()) <= 800 else -0.5  # 长度规则奖励
                if '</think>' in response:  # 思考相关规则奖励
                ...
                rewards[response_idx] -= rep_penalty(answer)  # 重复惩罚

                score = reward_model.get_score(messages, answer)  # 奖励模型打分
...
def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None, use_sglang=False):  # GRPO 训练一个 epoch（无 critic，用“组内相对奖励”当优势）
    for step, batch in enumerate(loader, start=start_step + 1):
        prompts = batch['prompt']  # list[str], length B
        prompt_inputs = tokenizer(prompts, return_tensors="pt", padding=True, return_token_type_ids=False,
                                  padding_side="left", add_special_tokens=False).to(args.device)  # 左 padding 编码
        if args.max_seq_len:         # 截断 prompt 到上限
            prompt_inputs["input_ids"] = prompt_inputs["input_ids"][:, -args.max_seq_len:]
            prompt_inputs["attention_mask"] = prompt_inputs["attention_mask"][:, -args.max_seq_len:]

        rollout_result = rollout_engine.rollout(  # 每个 prompt 采样 num_generations 条
        ...
        outputs = rollout_result.output_ids  # 完整序列 [B*num_gen, P+R]
        completion_ids = rollout_result.completion_ids
        completions = rollout_result.completions
        old_per_token_logps = rollout_result.per_token_logps.to(args.device).detach()  # 旧策略 logprob
        prompt_lens = rollout_result.prompt_lens.to(args.device)
        full_mask = (outputs != tokenizer.pad_token_id).long()
        logp_pos = prompt_lens.unsqueeze(1) - 1 + torch.arange(completion_ids.size(1), device=args.device).unsqueeze(0)  # 续写 token 在 logits 中的位置
        ...
        with autocast_ctx:           # 当前策略前向
            ...
            per_token_logps = F.log_softmax(res.logits[:, :-1, :], dim=-1).gather(2, outputs[:, 1:].unsqueeze(-1)).squeeze(-1).gather(1, logp_pos)  # 当前策略对续写 token 的 logprob

        with torch.no_grad():        # 参考策略 logprob（KL）
        ...
        grouped_rewards = rewards.view(-1, args.num_generations)  # [B, num_gen]  # 把同一 prompt 的若干条归为一组
        mean_r = grouped_rewards.mean(dim=1).repeat_interleave(args.num_generations)  # [B*num_gen]  # 组内均值
        std_r = grouped_rewards.std(dim=1, unbiased=False).repeat_interleave(args.num_generations)  # [B*num_gen]  # 组内标准差
        advantages = (rewards - mean_r) / (std_r + 1e-4)  # [B*num_gen]  # GRPO 优势 = 组内标准化奖励（无需 critic）

        completion_pad_mask = rollout_result.completion_mask.to(args.device).bool()  # 续写有效位置
        ...
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)]  # 首个 eos 下标
        completion_mask = ((torch.arange(is_eos.size(1), device=args.device).expand(is_eos.size(0), -1) <= eos_idx.unsqueeze(1)) & completion_pad_mask).int()  # [B*num_gen, R]  # 截到 eos（含）的损失掩码

        kl_div = ref_per_token_logps - per_token_logps  # log(π_ref/π)
        per_token_kl = torch.exp(kl_div) - kl_div - 1  # 无偏 KL 估计（k3）
        ratio = torch.exp(per_token_logps - old_per_token_logps)  # 重要性比值 π/π_old
        if args.loss_type == "cispo":  # CISPO：对比值做上界裁剪并 detach，再乘 logp（类似 REINFORCE）
            clamped_ratio = torch.clamp(ratio, max=args.epsilon_high).detach()
            per_token_loss = -(clamped_ratio * advantages.unsqueeze(1) * per_token_logps - args.beta * per_token_kl)
        else:                        # 标准 GRPO：PPO 式双裁剪取较小者，再减 KL 惩罚
            clipped_ratio = torch.clamp(ratio, 1 - args.epsilon, 1 + args.epsilon)
            per_token_loss1 = ratio * advantages.unsqueeze(1)
            per_token_loss2 = clipped_ratio * advantages.unsqueeze(1)
            per_token_loss = -(torch.min(per_token_loss1, per_token_loss2) - args.beta * per_token_kl)
        policy_loss = ((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1).clamp(min=1)).mean()  # 每条序列在有效位置求平均，再对 batch 求平均
        ...
        if step % args.save_interval == 0 or step == iters: rollout_engine.update_policy(model)  # 同步策略给采样引擎

        del prompt_inputs, outputs, completion_ids, per_token_logps, ref_per_token_logps  # 释放显存
        del completions, rewards, grouped_rewards, mean_r, std_r, advantages, completion_mask, completion_pad_mask, prompt_lens, logp_pos

    if step > start_step and step % args.accumulation_steps != 0:  # 尾部剩余梯度
        if args.grad_clip > 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
        optimizer.step()
        scheduler.step()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind GRPO (Group Relative Policy Optimization)")  # 组相对策略优化
    ...
    # Policy模型
    model, tokenizer = init_model(lm_config, base_weight, device=args.device)  # 策略模型
    # Reference模型
    ref_model, _ = init_model(lm_config, base_weight, device=args.device)  # 参考模型（冻结）
    ...
    total_optimizer_steps = math.ceil(iters / args.accumulation_steps) * args.epochs  # 调度器总步数（GRPO 不重复更新，故无 ppo_update_iters）
