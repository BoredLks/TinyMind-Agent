...
import torch                         # 训练框架
import torch.nn.functional as F      # softmax / kl_div / cross_entropy
...
from dataset.lm_dataset import SFTDataset  # 蒸馏也用对话数据
...
def distillation_loss(student_logits, teacher_logits, temperature=1.0, reduction='batchmean'):  # 知识蒸馏损失：让学生分布逼近教师分布（KL 散度）
    with torch.no_grad():            # 教师分布不参与求梯度
        teacher_probs = F.softmax(teacher_logits / temperature, dim=-1).detach()  # 教师概率（除以温度做软化）

    student_log_probs = F.log_softmax(student_logits / temperature, dim=-1)  # 学生对数概率（同样软化）

    kl = F.kl_div(                   # KL(teacher || student)
        student_log_probs,
        teacher_probs,
        reduction=reduction
    )
    return (temperature ** 2) * kl   # 乘 T^2 抵消软化对梯度幅度的缩放（标准蒸馏做法）


def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):  # 蒸馏训练一个 epoch
    start_time = time.time()
    last_step = start_step

    if teacher_model is not None:    # 教师固定为评估、不求梯度
        teacher_model.eval()
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        loss_mask = (labels[..., 1:] != -100).float()  # 只在“助手回答”位置（label≠-100）算损失的掩码
        lr = get_lr(epoch * iters + step, args.epochs * iters, args.learning_rate)  # 学习率
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

        # 前向传播（学生模型）
        with autocast_ctx:
            res = model(input_ids)   # 学生前向
            student_logits = res.logits[..., :-1, :].contiguous()  # 去掉最后一个位置（预测错位对齐）

        # 教师模型前向传播（只在eval & no_grad）
        if teacher_model is not None:
            with torch.no_grad():
                teacher_logits = teacher_model(input_ids).logits[..., :-1, :].contiguous()  # 教师 logits
                vocab_size_student = student_logits.size(-1)
                teacher_logits = teacher_logits[..., :vocab_size_student]  # 对齐词表维（容忍教师词表更大）

        # ========== 计算损失 ==========
        # 1) Ground-Truth CE Loss
        shift_labels = labels[..., 1:].contiguous()  # 右移一位的标签
        loss_mask_flat = loss_mask.view(-1)  # 展平掩码
        ce_loss = F.cross_entropy(   # 逐 token 交叉熵（先不归约）
            student_logits.view(-1, student_logits.size(-1)),
            shift_labels.view(-1),
            ignore_index=-100,
            reduction='none'
        )
        ce_loss_raw = torch.sum(ce_loss * loss_mask_flat) / (loss_mask_flat.sum() + 1e-8)  # 只在回答位置求平均
        if lm_config_student.use_moe: ce_loss = ce_loss_raw + res.aux_loss  # MoE 学生加辅助损失
        else: ce_loss = ce_loss_raw

        # 2) Distillation Loss
        if teacher_model is not None:  # 只在回答位置算蒸馏 KL
            ...
            distill_loss = torch.tensor(0.0, device=args.device)  # 无教师则蒸馏损失为 0

        # 3) 总损失 = alpha * CE + (1-alpha) * Distill
        loss = (alpha * ce_loss + (1 - alpha) * distill_loss) / args.accumulation_steps  # 加权组合并按累积步缩放
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():  # 保存（注意用学生配置）
            model.eval()
        ...
        del input_ids, labels, loss_mask, res, student_logits, ce_loss, distill_loss, loss  # 释放显存
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MiniMind Knowledge Distillation")  # 知识蒸馏
    ...
    lm_config_student = MiniMindConfig(hidden_size=args.student_hidden_size, num_hidden_layers=args.student_num_layers, use_moe=bool(args.student_use_moe))  # 学生配置
    lm_config_teacher = MiniMindConfig(hidden_size=args.teacher_hidden_size, num_hidden_layers=args.teacher_num_layers, use_moe=bool(args.teacher_use_moe))  # 教师配置
    ...
    # ========== 5. 定义学生和教师模型 ==========
    model, tokenizer = init_model(lm_config_student, args.from_student_weight, device=args.device)  # 学生（被训练）
    Logger(f'学生模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, device=args.device)  # 教师（提供软标签）
    teacher_model.eval()
    teacher_model.requires_grad_(False)  # 冻结教师
